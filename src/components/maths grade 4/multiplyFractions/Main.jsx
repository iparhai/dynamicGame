...
function Main(props) {
    ...
    useEffect(() => {
        setHintState("Multiply the following fractions amd then make teeths accourding to answer")
    });
    ...
    let computedNumerator = (qnum1*qnum2);
    let computedDenominator = dif=='b'?(qden1*qden2):(qden2);
    ...
    return (
        <div className='multiplyCartoon' style={{backgroundImage: `url(${problem.background})`}}>
            <div className='showQuestion'>
                {
                    ...
                    :
                        <h5>{problem.question} {qnum1} with {qnum2} by {qden2}</h5>
                }
            </div>

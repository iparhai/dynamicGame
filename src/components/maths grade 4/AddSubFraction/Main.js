...
function Main(props) {
    ...
    const [qnum1,setqnum1] = useState(problem.numerator[Nindex]+1);
    const [qden1,setqden1] = useState(problem.denominator[Dindex]);
    const [qnum2,setqnum2] = useState(problem.numerator[Nindex]);
    const [qden2,setqden2] = useState(problem.denominator[Dindex]+1);
    const [hintState, setHintState] = useContext(GlobalState);

    useEffect(() => {
        setHintState("Subtract the fraction shown and then make  teeeths according by dragging the dentures to make the answer")
    });
    ...
    // computedDenominator will calculate lCM
    let computedNumerator = problem.operator == '-' ? ((qnum1*qden2)-(qnum2*qden1)) : ((qnum1*qden2)+(qnum2*qden1));
    let computedDenominator = (qden1 > qden2) ? qden1 : qden2;
    while (true) {
        if (computedDenominator % qden1 == 0 && computedDenominator % qden2 == 0) {
            console.log(`The LCM of ${qden1} and ${qden2} is ${computedDenominator}`);
            break;
        }
        computedDenominator++;
    }
    ...
    // console.log("Numerator" + computedNumerator);
    // console.log("Denominator "+ computedDenominator);
    // console.log("Upper" + upperJaw.length);
    // console.log("Lower" + lowerJaw.length);
    return (
        <div className='addSubCartoon' style={{backgroundImage: `url(${problem.background})`}}>
            <div className='showQuestion'>
                <h5>{problem.question} {qnum1} by {qden1} with {qnum2} by {qden2}</h5>
            </div>
